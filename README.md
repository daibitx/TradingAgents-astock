...
| 特性 | 原版 TradingAgents | hsliuping/TradingAgents-CN | **本 Fork** |
|------|-------------------|--------------------------|-------------|
| 许可证 | Apache 2.0 | app/ + frontend/ 商业闭源 | **全 Apache 2.0** |
| 部署依赖 | pip install | MongoDB + Redis 必须 | **pip install 即跑** |
| A 股数据 | ❌ | Tushare（积分墙） | **mootdx + 腾讯 + akshare（免费）** |
| A 股特化角色 | ❌ | 基础翻译 | **政策/游资/解禁 3 个深度角色** |
| A 股交易约束 | ❌ | 部分 | **T+1/涨跌停/手数/ST 全覆盖** |
| 配套教程 | 英文文档 | 仅代码 | **EP12-EP16 视频系列** |
...
以下是使用 Kimi 2.6 对科创板股票 688017（绝味食品-SH）的分析结果摘要：
